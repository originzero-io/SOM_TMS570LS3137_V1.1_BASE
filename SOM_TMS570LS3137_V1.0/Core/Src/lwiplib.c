...
{
    results_enum_t return_value = RESULT_ERR_UNKNOWN;
    ...
    hdkif_driver_set_mac_addr(instance_number, mac_address);

    if (NULL_PTR
            == netif_add(&hdkNetIF[instance_number], ipadd_p, netmask_p, gw_p,
                         NULL, hdkif_init, ethernet_input)) // 'ethernetif_init' and 'ethernet_input' should be defined according to your Ethernet driver
    {
    ...
    {
#if LWIP_DHCP
        while (!dhcp_supplied_address(&hdkNetIF[instance_number]))
        {
            dhcp_start(&hdkNetIF[instance_number]);
            int i = 9999999;

            uint8_t status = dhcp_supplied_address(&hdkNetIF[instance_number]);
            while (1) // Wait for DHCP to bind
            {
                // Here, you would usually call a function to pass time, like servicing
                // your watchdog, or if in an OS, yielding the thread.
    ...
    else if (ip_mode == IPADDR_USE_AUTOIP)
    {
        ...
        while (hdkNetIF[instance_number].ip_addr.addr == 0) // Wait for AutoIP to bind
        {
